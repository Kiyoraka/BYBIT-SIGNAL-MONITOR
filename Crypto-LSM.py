...
class BybitMonitor:
    ...
    def check_heikin_ashi_signal(self, ha_df, lookback=3):
        """Check for bullish Heikin Ashi pattern"""
        ...
        # Check if all last n candles are bullish (close > open)
        bullish_candles = all(
    ...
    def calculate_volume_profile(self, df):
        ...
        try:
            ...
            return {
                ...
                'high_volume_bullish': high_volume.iloc[-1] and df['price_change'].iloc[-1] > 0  # Changed to positive price change
            }
        except Exception as e:
        ...
        try:
            ...
            if self.config.get('rsi_enabled', 1):
                ...
                if df_4h is not None and not df_4h.empty:
                    ...
                    if rsi is not None:
                        ...
                        if current_rsi < self.config['rsi_threshold']:  # Changed from > to 
            ...
            if self.config.get('bb_enabled', 1):
                ...
                if df_bb is not None and not df_bb.empty:
                    ...
                    if bb_data is not None:
                        if bb_data['percentage'].iloc[-1] * 100 < self.config['bb_percentage_threshold']:  # Changed from > to 
                            triggered_indicators += 1
            ...
            if self.config.get('volume_enabled', 1):
                ...
                if df_vol is not None and not df_vol.empty:
                    ...
                    if volume_data is not None and volume_data['high_volume_bullish']:  # Changed from bearish to bullish
                        triggered_indicators += 1
                    ...
                    for result in results:
                        ...
                        for indicator, value in result['signals'].items():
                            if indicator == 'rsi':
                                ...
                                print(f"Heikin Ashi ({self.config['heikin_timeframe']}): {value} bullish candles")
                            elif indicator == 'macd':
